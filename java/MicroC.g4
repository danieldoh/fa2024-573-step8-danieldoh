...
if_stmt returns [IfStatementNode node] : 'if' '(' cond ')' '{' thenBlock=statements '}' ( 'else' '{' elseBlock=statements '}' { $node = new IfStatementNode($cond.node, $thenBlock.node, $elseBlock.node); } 
          | { $node = new IfStatementNode($cond.node, $thenBlock.node, null); });


else_stmt returns [StatementListNode node] : 'else' '{' elseBlock=statements '}' {$node = $elseBlock.node;}
          | {$node = null;};
...
cast_expr returns [CastNode node] : '(' type ')' expr {$node = new CastNode($type.t, $expr.node);};
